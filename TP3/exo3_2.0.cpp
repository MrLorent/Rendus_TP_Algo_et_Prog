...
int balance(BinarySearchTree* node)
{
...
{    
    ...
    void insertNumber(int aValue) {
		...
		// cette condition peut sauter, tu peux faire le if juste en dessous puis un else if pour == 2
		if(rightHeight - leftHeight == -2 || rightHeight - leftHeight == 2){
            if(rightHeight - leftHeight == -2){
				// pas besoin de tester la hauteur, si y a un déséquilbre de 2 ou -2 la taille est au moins 3
				// tu peux utiliser les régles listées dans le sujet
				// if (right_balance == 1)
				...
				if(this->height() == 3){
                    ...
                    if(this->left->get_left_child()){
                        leftHeight = this->left->get_left_child()->height();
                    }

                    if(this->left->get_right_child()){
                        rightHeight = this->left->get_right_child()->height();
                    }
                ...
                if(this->height() == 3){
                    ...
                    if(this->right->get_left_child()){
                        leftHeight = this->right->get_left_child()->height();
                    }

                    if(this->right->get_right_child()){
                        rightHeight = this->right->get_right_child()->height();
                    }
    ...
    void rightRotate(){
		// Pour simplifier, considere qu'on a bien un enfant gauche (on fait cette rotation à cause d'un déséquilibre gauche)
		// creer un nouveau noeud avec comme valeur: this->value, comme enfant droit: this->right et
		// comme enfant gauche: this->left->right

		// this->value = this->left->value
		// this->left = this->left->left
		// this->right = nouveau noeud
...
Node* createNode(BinarySearchTree* source) {
	BinarySearchTree* node = new BinarySearchTree(source->value);
